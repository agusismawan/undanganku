...
export function setupRSVP() {
    ...
    function renderTimeline(page) {
          ...
          <div class="timeline-card" style="display:flex; flex-direction:column; gap:8px; padding:12px; border-bottom:1px solid #ddd;">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
              <div style="display:flex; align-items:flex-start; gap:10px;">
                ...
                <div>
                  <div style="font-size:14px; font-weight:600;">${item.name
                    }</div>
        ...
        if (totalPages > 1) {
            ...
            // Prev
            pagHtml += `<button type="button" ${page === 1 ? "disabled" : ""
            ...
            for (let i = 0; i < pages.length; i++) {
                ...
                pagHtml += `<button type="button" ${curr === page ? 'class="active"' : ""
                    } onclick="window.gotoTimelinePage(${curr}, event)">${curr}</button>`;
            }

            // Next
            pagHtml += `<button type="button" ${page === totalPages ? "disabled" : ""
                } onclick="window.gotoTimelinePage(${page + 1}, event)">></button>`;
